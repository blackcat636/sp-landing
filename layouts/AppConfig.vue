...
// onMounted(()=>{
//   onChangeTheme(config.value.theme,config.value.darkTheme)
// })
//
//
// watch(
//     ()=>settingsStore.config.value,
//     () => {
//   console.log(config.value.theme,config.value.darkTheme);
//   onChangeTheme(config.value.theme,config.value.darkTheme);
// });
watch(config.value,() => {
  onChangeTheme(config.value.theme,config.value.darkTheme);
...
console.log('onChangeTheme')
  ...
  cloneLinkElement.addEventListener('load', () => {
      ...
      settingsStore.changeThemeSettingsTest(theme, mode === 'dark');
    });
...
<template>
    ...
    <Sidebar v-model:visible="visible" position="right" class="layout-config-sidebar w-20rem">
        <h5>Scale</h5>{{config.scale}}
        <div class="flex align-items-center">
            <Button icon="pi pi-minus" type="button" @click="config.scale--" class="p-button-text p-button-rounded w-2rem h-2rem mr-2" :disabled="layoutConfig.scale.value === scales[0]"></Button>
            ...
            <Button icon="pi pi-plus" type="button" @click="config.scale++" class="p-button-text p-button-rounded w-2rem h-2rem ml-2" :disabled="layoutConfig.scale.value === scales[scales.length - 1]"></Button>
        </div>
